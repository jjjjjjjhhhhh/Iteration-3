	...
	<div>
		<div class="center_view">
			...
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.nickname" placeholder="UserName"
								clearable>
							</el-input>
						</div>
					</div>
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.content" placeholder="Comment content"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('discussjiankangzhishi','新增')">
			...
			<el-table
				...
				<el-table-column label="Comment content" min-width="140" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<span v-html="scope.row.content"></span>
					</template>
				</el-table-column>
				<el-table-column label="Reply content" min-width="140" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<span v-html="scope.row.reply"></span>
					</template>
				</el-table-column>
                <el-table-column v-if="btnAuth('discussjiankangzhishi','修改')" label="Disable account" :resizable='true' :sortable='false' align="left" header-align="left">
                    <template #default="scope">
				...
				<el-table-column label="operate" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button class="edit_btn" type="primary" @click="editClick(scope.row.id)" v-if=" btnAuth('discussjiankangzhishi','修改')">
							<i class="iconfont icon-xiugai5"></i>
							Edit						</el-button>
						<el-button class="del_btn" type="danger" @click="delClick(scope.row.id)" >
							<i class="iconfont icon-shanchu4"></i>
							Delete						</el-button>
						<el-button class="operate_btn" v-if="btnAuth('discussjiankangzhishi','View comments')" type="warning" @click="commentClick(scope.row.id)">
...
<script setup>
	...
	const tableName = 'discussjiankangzhishi'
	const formName = 'Health Knowledge Comment Form'
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussdiscussjiankangzhishi?refid=' + id)
	}
    ...
    const updateItem = (row)=>{
        context.$http.post(`${tableName}/update`,row).then(res=>{
            if(res.data.code==0){
                context.$message.success("Update success")
            }
        })
    }
    const back = ()=>{
