<template>
    <div>
        ...
        <el-dialog
            title="Modify parent menu"
            v-model="dialogVisible"
            width="30%">
            <div style="display: flex;justify-content: center;align-items: center;margin-bottom: 20px">
                <div>Parent menu:</div>
                <el-select v-model="n_first_index" placeholder="Please select the parent menu" style="width: 200px">
                    <el-option
            ...
            <div style="width: 100%;text-align: center">
                <el-button type="primary" @click="moveTo">Determine</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
const save = ()=>{
    context.$http.post('menu/update',{
        ...
        if(res.data.code==0){
            ElMessage.success('Saved successfully, takes effect after logging in again')
        }
    })
}
...
const changeName = (row,arr)=>{
    ElMessageBox.prompt('Enter new menu name: modify menu name',{
        inputValidator:(value)=>{
            if(!value || !value.trim())return 'Enter menu name'
            if(arr.find(item=>item.menu==value.trim())){
                return 'The menu name already exists'
            }
        },
    }).then(({value})=>{
        row.menu = value.trim()
