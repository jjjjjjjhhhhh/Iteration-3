			...
			<div class="thumbs_view">
				...
				<template v-else>
					<div class="zan_view zanActive" v-if="thumbsupOrCrazilyInfo.type==21" @click="cancelThumbsupOrCrazilyClick(21)">
						...
						<span class="textActive">Cancel Like({{detail.thumbsupNumber}})</span>
					</div>
					<div class="zan_view can_view zanActive" v-else @click="cancelThumbsupOrCrazilyClick(22)">
						...
						<span class="textActive">Cancel Dislike({{detail.crazilyNumber}})</span>
					</div>
				</template>
			</div>
			...
			<div class="info_view">
				<div class="title_view">
					...
					<div class="collect_view" v-if="collectType" @click="collectClick(-1)">
						...
						<span class="textActive">Cancel Collect</span>
					</div>
				</div>
				<div class="info_item">
					<div class="info_label">Required Ingredients</div>
					<div  class="info_text" >{{detail.suoxushicai}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Recipe Benefits</div>
					<div  class="info_text" >{{detail.shipugongxiao}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Suitable For</div>
					<div  class="info_text" >{{detail.shiherenqun}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Preparation Method</div>
					<div  class="info_text" >{{detail.zhizuofangfa}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">Comment Count</div>
					<div  class="info_text" >{{detail.discussNumber}}</div>
				</div>
				<div class="btn_view">
					<el-button class="edit_btn" v-if="centerType&&btnAuth('jiankangshipu','修改')" type="primary" @click="editClick">Edit</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('jiankangshipu','删除')" type="danger" @click="delClick">Delete</el-button>
				</div>
			</div>
		</div>
		<div class="detail-box">
			<div class="title"><span>Details</span></div>
			<div class="content" v-html="detail.xiangqingjianjie"></div>
		</div>
				...
				<div class="my_comment_view">
					<el-form ref="commentFormRef" :model="commentForm" class="my_comment_form"
						...
						<el-form-item prop="content" class="editor">
                            <editor :value="commentForm.content" placeholder="Kind words create good karma, harsh words hurt hearts"
                                    class="list_editor" @change="contentChange"></editor>
						</el-form-item>
					</el-form>
					<div class="comment_btn">
						<el-button class="add_btn" type="primary" @click="commentSave">Comment Now</el-button>
						<el-button class="reset_btn" @click="resetForm">Reset</el-button>
					</div>
				</div>
					...
					<div class="comment" v-for="(item,index) in commentList" :key="index">
						<div class="comment_top">
							<div class="comment_user">
								<div class="comment_user_img">
                                    <img v-if="item.avatarurl" :src="$config.url + item.avatarurl" alt="">
						...
						<div class="comment_bottom">
                            ...
                            <div v-if="item.userid==user.id" class="comment_action">
                                <span class="del" @click="commentDel(item)" style="cursor: pointer">Delete</span>
                            </div>
...
<script setup>
	...
	const cancelThumbsupOrCrazilyClick = (type) => {
		...
		context?.$http({
			...
			getThumbsupOrCrazily()
			context?.$toolUtil.message('Cancel successfully', 'success')
		})
	}
	// 收藏
	...
	const commentSave = async () => {
        if(!commentForm.value.content ||commentForm.value.content=='<p><br></p>'){
            return context.$message.error("Please enter the comment content")
        }
		let sensitiveWords = "";
		...
		commentFormRef.value.validate((valid) => {
			if (valid) {
				context?.$http({
					...
					context?.$toolUtil.message('Comment successful', 'success')
                    resetForm()
                    getCommentList()
				})
			}
    ...
    const commentDel = (item)=>{
        context.$confirm("Are you sure you want to delete this comment?","prompt").then(()=>{
            context.$http.post(`discuss${tableName}/delete`,[item.id]).then(res=>{
                if(res.data.code==0){
                    context.$message.success("Delete successfully")
                    context.$http.get(`${tableName}/info/${detail.value.id}`).then(res=>{
                        let detail = res.data.data
                        detail.discussNumber--
                        context.$http.post(`${tableName}/update`,detail).then(()=>{
	...
	//删除
	const delClick = () => {
		ElMessageBox.confirm(`Do you want to delete this${formName}？`, 'prompt', {
