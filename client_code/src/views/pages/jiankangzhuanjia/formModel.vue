	...
	<div class="detail-page" :style='{}'>
			...
			<div class="info_view">
				<div class="title_view">
					<div class="detail_title">
						<span>{{detail.zhuanjiaxingming}}</span>
					</div>
				</div>
				<div class="info_item">
					<div class="info_label">Expert Account</div>
					<div  class="info_text" >{{detail.zhuanjiazhanghao}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">Age</div>
					<div  class="info_text" >{{detail.nianling}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Expertise</div>
					<div  class="info_text" >{{detail.shanzhanglingyu}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">Personal Profile</div>
					<div  class="info_text" >{{detail.gerenjianjie}}</div>
				</div>
				<div class="btn_view">
					<el-button class="edit_btn" type="primary" v-if="btnAuth('jiankangzhuanjia','私信')&&detail.id!=user.id" @click="chatClick">Message</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('jiankangzhuanjia','修改')" type="primary" @click="editClick">Edit</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('jiankangzhuanjia','删除')" type="danger" @click="delClick">Delete</el-button>
				</div>
			</div>
		</div>
		<el-dialog v-model="chatVisible" @close="clearChat" :title="fname">
			...
			<div slot="footer" class="dialog-footer">
				<el-input size="small" @keydown.enter.native="addChat" v-model="chatForm.content" placeholder="Please enter content" style="width: calc(100% - 240px);float: left;border: 1px solid #ccc;margin-right: 10px;">
				</el-input>
				<el-button size="small" :disabled="chatForm.content?false:true" type="primary" @click="addChat">Send</el-button>
				<el-upload style="display: inline-block;margin: 0 6px;" class="upload-demo" :action="uploadUrl" :on-success="uploadSuccess"
					...
					<el-button size="small" type="success">Upload File</el-button>
				</el-upload>
			</div>
		</el-dialog>
	</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
            ...
            title.value = res.data.data.zhuanjiaxingming
            ...
            if(res.data.data.zhuanjiazhanghao){
                fname.value = res.data.data.zhuanjiazhanghao
            }
