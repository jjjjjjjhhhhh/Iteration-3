	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="btn_view">
					<el-button class="edit_btn" type="primary" v-if="btnAuth('yonghu','私信')&&detail.id!=user.id" @click="chatClick">private message</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('yonghu','修改')" type="primary" @click="editClick">Edit</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('yonghu','删除')" type="danger" @click="delClick">Delete</el-button>
				</div>
			</div>
		</div>
		<el-dialog v-model="chatVisible" @close="clearChat" :title="fname">
			...
			<div slot="footer" class="dialog-footer">
				<el-input size="small" @keydown.enter.native="addChat" v-model="chatForm.content" placeholder="Please enter the content" style="width: calc(100% - 240px);float: left;border: 1px solid #ccc;margin-right: 10px;">
				</el-input>
				...
				<el-upload style="display: inline-block;margin: 0 6px;" class="upload-demo" :action="uploadUrl" :on-success="uploadSuccess"
					...
					<el-button size="small" type="success">Upload files</el-button>
				</el-upload>
			</div>
		</el-dialog>
	</div>
	...
	const getDetail = () => {
		context?.$http({
            ...
            if(res.data.data.yonghuzhanghao){
                fname.value = res.data.data.yonghuzhanghao
            }
